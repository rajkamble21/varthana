...
const deleteUserById = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `User with id:${req.params.id} deleted successfully!`, user });
    } catch (error) {
        return res.status(500).json({ message: error.message, error });
    }
}
...
const addUsersInBulk = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `${createdUsers.length} users added, ${alreadyExistingUsers.length} users already exists`, createdUsers, alreadyExistingUsers });
    } catch (error) {
        return res.status(500).json({ message: error.message, error });
    }
}
